...
function pShuffle() {
	...
	for (i = 0; i < floor(windowWidth / 30); i++)
		arrVal.push(Math.ceil(random(height - 200)));
}
...
function draw() {
	...
	for (let k = 0; k < arrVal.length; k++) {
		...
		switch (sortAlgo.value()) {
			case "Bubble Sort":
				k === j || k === j + 1 ? fill(marker) : fill(normal);
				break;

			case "Selection Sort":
				k === j || k === t ? fill(marker) : fill(normal);
				break;

			case "Insertion Sort":
				k === i || k === j ? fill(marker) : fill(normal);
				break;
			...
			case "Quick Sort":
				states[k] === 0 ? fill(marker) : fill(normal);
				break;
